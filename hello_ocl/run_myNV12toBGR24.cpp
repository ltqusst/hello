...
void save_bmp(const char * filename, int w, int h, uint8_t * pBGR)
{
	bmpfile_t *bmp;
	int x, y;

	if ((bmp = bmp_create(w, h, 24)) == NULL) {
	...
	for (x=0;x<w;x++)
	{
		bmp_set_pixel(bmp, x, y, rgb_pixel_t{pBGR[0], pBGR[1], pBGR[2]});
		pBGR += 3;
	}
...
void run_myNV12toBGR24(cl_platform_id platform_id, const char * kernel_filename)
{
    ...
    img<uint8_t> D(M, N, img<uint8_t>::BGR24);
	...
	dt.stop("c_version");
	printf("CPU version complete %.6f sec\n", dt["c_version"]);
	save_bmp("BGR24(CPU).bmp", M, N, D);
    ...
    cl_mem bufB = clCreateBuffer(context, CL_MEM_READ_WRITE,  B.size, NULL, NULL);
